...
from tutorial_ardusub.pid_controller import PIDController
...
class DepthPIDController(Node):
    def __init__(self):
        ...
        self.pid = PIDController(kp=70.0, ki=2.5, kd=5,
                                 setpoint=0.0, dt=0.1)
        ...
        self.sub_pressure = self.create_subscription(
            FluidPressure, 'pressure', self.depth_callback, 10)
    ...
    def control_loop(self):
        ...
        self.pub.publish(m)
        self.get_logger().info(
            f"[PID] depth={self.depth:.3f} m  setpt={self.pid.setpoint:.3f} m  → thrust z={m.z:.3f}"
        )
