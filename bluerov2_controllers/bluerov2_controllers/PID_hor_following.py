...
class FollowHorizontalLane(Node):
    def __init__(self):
        ...
        self.yaw_pid = PIDController(kp=2.0, ki=0.1, kd=0.75,
                                     setpoint=0.0, dt=0.1)
        # Depth PID keeps lane centered vertically
        ...
        # Subscribe to the camera to learn image height
        ...
        # Publisher for manual control commands
        ...
        # Latest lane measurements
        self._latest_angle = None      # from best_horizontal_lane[1]
        self._latest_offset = None     # normalized vertical offset

        # Run control loop at 10 Hz
        self.create_timer(0.1, self.control_loop)

        self.get_logger().info(
            "FollowHorizontalLane ready. Listening on 'bluerov2/camera' and '/lane_detector/best_horizontal_lane'"
        )

    def _image_cb(self, img_msg: Image):
        # Capture image height once for normalization
    ...
    def lane_cb(self, msg: Float64MultiArray):
        ...
        # Normalize y offset to range ±1
        self._latest_offset = (y_center - half) / half

    def control_loop(self):
        # Ensure we have both angle and offset
        ...
        # Log by stage
        if not centered:
            self.get_logger().debug(f"[Stage 1] Adjust depth → z={z_cmd:.2f}")
        elif not aligned:
            self.get_logger().debug(f"[Stage 2] Adjust yaw   → r={r_cmd:.2f}")
        else:
            self.get_logger().info(f"[Stage 3] Forward      → x={x_cmd:.2f}")
